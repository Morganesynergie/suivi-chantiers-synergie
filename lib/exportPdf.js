...
export async function generatePdfBlob(html, { pageNumbers = false } = {}) {
  ...
  try {
    ...
    for (const el of avoidBreakEls) {
      ...
      if (height >= PAGE_H) continue; // ne tiendrait de toute façon pas sur une seule page
      const topPage = Math.floor(top / PAGE_H);
      const bottomPage = Math.floor((top + height - 1) / PAGE_H);
      if (bottomPage > topPage) {
        const nextPageTop = (topPage + 1) * PAGE_H;
