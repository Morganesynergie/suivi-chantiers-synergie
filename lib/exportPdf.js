...
export async function openGeneratedPdf(html, { fileName = "export.pdf", onError, pageNumbers = false } = {}) {
  try {
    ...
    // Le titre PDF (pdf.setProperties ci-dessus) ne suffit pas partout : sur
    // le poste de Morgane, le lecteur PDF intégré (Adobe Acrobat en plugin
    // navigateur) ignore ce Titre pour le nom par défaut de "Enregistrer
    // sous" et propose à la place l'identifiant brut de l'URL blob:
    // (un UUID sans rapport). Envelopper le Blob dans un objet File nommé
    // avant de créer l'URL objet corrige ça : la plupart des lecteurs
    // (Chrome PDFium, Adobe Acrobat, Edge) lisent alors le nom du File pour
    // suggérer le nom de fichier à l'enregistrement.
    const file = new File([blob], fileName, { type: "application/pdf" });
