...
export async function generatePdfBlob(html, { pageNumbers = false, title } = {}) {
  ...
  try {
    ...
    // que d'être coupés.
    // Petite marge de sécurité ajoutée au-delà de la limite de page stricte
    // quand on repousse un bloc "avoid-break" : sans elle, le bloc atterrit
    // avec un écart nul par rapport à la limite calculée ici (en pixels CSS,
    // AVANT rendu), alors que le découpage final se fait sur l'image rasterisée
    // par html2canvas (APRÈS rendu) — un tout petit écart de rendu entre les
    // deux (arrondi, métriques de police) suffit alors à retranscher le haut
    // du bloc (souvent son titre) sur la page précédente. C'est ce qui
    // provoquait "le titre de l'Article 7 est coupé en deux entre les pages
    // 3 et 4" : le bloc était bien repoussé, mais pile sur la limite, sans
    // aucune marge d'erreur.
    const PAGE_BREAK_SAFETY = 8;
    ...
    for (const el of avoidBreakEls) {
      ...
      if (bottomPage > topPage) {
        const nextPageTop = (topPage + 1) * CONTENT_H + PAGE_BREAK_SAFETY;
    ...
    // Le nom qu'on passe à shareOrDownloadBlob (attribut `download` de l'ancre)
    // ne sert qu'au flux PWA/partage — quand le PDF s'ouvre dans un nouvel
    // onglet (cas desktop normal, voir openGeneratedPdf), le bouton
    // "Enregistrer" du lecteur PDF intégré de Chrome ignore totalement ce nom
    // : il propose par défaut le Titre inscrit dans les métadonnées internes
    // du PDF lui-même (ou, à défaut, un nom générique du type "download").
    // D'où "le PDF ne s'enregistre pas sous le nom demandé" — corrigé en
    // posant ce même nom ici, dans le document PDF, pas seulement sur l'ancre
    // de téléchargement.
...
export async function openGeneratedPdf(html, { fileName = "export.pdf", onError, pageNumbers = false } = {}) {
  try {
    ...
    const blob = await generatePdfBlob(html, { pageNumbers, title });
    if (needsPdfShareFlow()) {
      await shareOrDownloadBlob(blob, fileName);
      return;
    }
    // Ni le Titre PDF (pdf.setProperties ci-dessus) ni un Blob enveloppé
    // dans un File nommé ne suffisent : sur le poste de Morgane, le lecteur
    // PDF intégré (plugin Adobe Acrobat) ignore les deux pour le nom par
    // défaut de "Enregistrer sous" et affiche à la place l'identifiant brut
    // de l'URL blob: (un UUID sans rapport) — confirmé après coup, à deux
    // reprises. Seul un en-tête HTTP Content-Disposition, lui, est respecté
    // par tous les lecteurs (c'est une donnée standard, pas une convention
    // propre à un visualiseur) : on ouvre donc une fenêtre vide tout de
    // suite (pour ne pas se faire bloquer comme pop-up une fois le PDF
    // généré), puis on y fait naviguer un vrai POST HTTP vers
    // /api/pdf-preview/<nom>, qui renvoie le PDF avec ce Content-Disposition
    // (voir cette route) — cf. shareOrDownloadBlob plus haut, qui elle
    // fonctionne déjà bien via l'attribut `download` d'une ancre, mais ne
    // s'applique qu'au flux de téléchargement direct PWA, pas à l'aperçu en
    // onglet utilisé ici.
    const win = window.open("", "_blank");
    if (!win) {
    ...
    try {
      ...
      dt.items.add(new win.File([blob], fileName, { type: "application/pdf" }));
      const form = doc.createElement("form");
      form.method = "POST";
      form.action = `/api/pdf-preview/${encodeURIComponent(fileName)}`;
      ...
      // Navigateur trop ancien pour DataTransfer/File dans une autre
      // fenêtre : on retombe sur l'ancienne méthode (URL blob nommée) dans
      // la fenêtre déjà ouverte, plutôt que de la laisser vide.
      console.error("openGeneratedPdf form-post fallback", innerErr);
      const file = new File([blob], fileName, { type: "application/pdf" });
