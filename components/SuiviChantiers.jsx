...
// Habillage visuel par type de marché (bloc "Suivi facturation" de la fiche
// chantier — voir renderMarcheBlock) : un marché principal, un TS et un
// bloc PRORATA doivent se distinguer d'un coup d'œil sur un chantier qui
// cumule plusieurs blocs, comme demandé par Morgane. Le violet du PRORATA
// existait déjà ; principal et TS reçoivent chacun leur propre teinte
// (nom du bloc, pastille "HT marché" via Pill, liseré du tableau).
const MARCHE_COLOR_SCHEME = {
  principal: { name: COLORS.accent, pill: "accent", headerBg: "#EFF5FB", border: "#CFE1F2" },
  ts: { name: "#0F766E", pill: "teal", headerBg: "#F0FBFA", border: "#B7E4DC" },
  prorata: { name: "#8B5CF6", pill: "purple", headerBg: "#F5F3FF", border: "#DDD6FE" },
};
...
// Écrit un nombre entier (0 à 999 999 999) en toutes lettres, en français —
// utilisé pour la ligne "Net à payer en toutes lettres" de l'appel d'avance
// de démarrage (voir generateAvancePdf). "cent" et "quatre-vingts" ne
// prennent leur "s" que lorsqu'ils terminent vraiment le nombre entier (rien
// ne les suit, même pas un "mille"/"million" plus loin) — d'où le paramètre
// isTerminal propagé depuis le chunk le plus significatif jusqu'au dernier
// chunk non nul.
function nombreEnLettresFr(n) {
  if (n === 0) return "zéro";
  const PETITS = ["zéro", "un", "deux", "trois", "quatre", "cinq", "six", "sept", "huit", "neuf", "dix",
    "onze", "douze", "treize", "quatorze", "quinze", "seize", "dix-sept", "dix-huit", "dix-neuf"];
  const DIZAINES = { 2: "vingt", 3: "trente", 4: "quarante", 5: "cinquante", 6: "soixante" };

  function twoDigits(n2, isTerminal) {
    if (n2 < 20) return PETITS[n2];
    if (n2 < 70) {
      const d = Math.floor(n2 / 10), u = n2 % 10;
    ...
    if (n2 < 80) {
      ...
      return "soixante-" + PETITS[u];
    }
    const u = n2 - 80;
    if (u === 0) return isTerminal ? "quatre-vingts" : "quatre-vingt";
    return "quatre-vingt-" + PETITS[u];
  }

  function chunk999(n3, isTerminal) {
    const c = Math.floor(n3 / 100);
    const rest = n3 % 100;
    const parts = [];
    if (c > 0) {
      if (c === 1) parts.push("cent");
      else parts.push(PETITS[c] + " cent" + (rest === 0 && isTerminal ? "s" : ""));
    }
    if (rest > 0) parts.push(twoDigits(rest, isTerminal));
    return parts.join(" ");
  }
  ...
  const terminalIdx = chunks.findIndex((c) => c !== 0);
  const SCALES = ["", "mille", "million", "milliard"];
  const words = [];
  for (let i = chunks.length - 1; i >= 0; i--) {
    const c = chunks[i];
    if (c === 0) continue;
    const isTerminal = i === terminalIdx;
    if (i === 1 && c === 1) {
      words.push("mille");
    } else {
      let w = chunk999(c, isTerminal);
      if (i > 0) w += " " + SCALES[i] + (i >= 2 && c > 1 ? "s" : "");
      words.push(w);
    }
  }
  return words.join(" ");
...
function montantEnLettresFr(montant) {
  const totalCentimes = Math.round((montant || 0) * 100);
  const euros = Math.floor(totalCentimes / 100);
  const centimes = totalCentimes % 100;
  let out = nombreEnLettresFr(euros);
  out = out.charAt(0).toUpperCase() + out.slice(1);
  // "million(s)"/"milliard(s)" appellent "d'euros" (pas juste "euros") quand
  // ils terminent le nombre — ex. "un million d'euros", pas "un million euros".
  if (/\b(millions?|milliards?)$/.test(out)) out += euros > 1 ? " d'euros" : " d'euro";
  else out += euros > 1 ? " euros" : " euro";
  if (centimes > 0) {
    out += " et " + nombreEnLettresFr(centimes) + (centimes > 1 ? " centimes" : " centime");
  }
  return out;
}
...
// Coordonnées de l'entête/pied de page SYNERGIE BTP, reprises telles quelles
// du modèle "Facture d'avance de démarrage" déjà utilisé par Morgane
// (Batigest) pour que le document généré ici ait la même présentation que
// celui qu'elle envoie déjà à ses clients/banques.
const SYNERGIE_BTP_COORDS = {
  adresse: "374 La Grippière - 97170 Petit-Bourg",
  siret: "830 962 668 00018",
  tvaIntra: "F44830962668",
  tel: "0590 26 85 04",
  email: "synergiebtp971@gmail.com",
  capital: "12 000 €",
  ape: "4312A",
...
// permet pas de facturer un acompte) — c'est une simple demande de
// trésorerie que le client règle directement, avant que le montant ne
// vienne en déduction (ligne "acompte"/ADD) du total à solder sur les
// situations. Le document généré reprend la présentation du modèle que
// Morgane utilise déjà pour ce même usage (voir SYNERGIE_BTP_COORDS
...
function AvanceDemarragePdfModal({ chantier, marche, onClose, onGenerated }) {
  ...
  function generate() {
    setGenError("");
    const destHtml = (destinataire || "Client non renseigné")
      .split("\n").map((l) => l.trim()).filter(Boolean).map((l) => `<div>${l}</div>`).join("");
    const html = `
      <html><head><title>Avance de démarrage — ${chantier.titre}</title>
      <style>
        body{font-family:system-ui,sans-serif;color:#16233B;padding:32px;font-size:12.5px;}
        ...
        @media (max-width:480px){ .close-bar{padding:8px 12px;} .close-bar-label{display:none;} .close-bar-brand img{height:20px;} .print-btn,.close-btn{padding:7px 11px;font-size:12px;} }
        ...
        .company-box{text-align:right;font-size:11px;color:#3A4558;line-height:1.5;}
        .company-box .company-name{font-size:16px;font-weight:700;color:#16233B;margin-bottom:2px;}
        .doc-box{border:1.5px solid #16233B;border-radius:6px;overflow:hidden;margin-bottom:0;}
        .doc-row{display:flex;border-bottom:1.5px solid #16233B;}
        .doc-cell{padding:10px 14px;}
        .doc-cell-label{width:230px;flex-shrink:0;border-right:1.5px solid #16233B;font-size:11px;}
        .doc-cell-label .kind{font-weight:700;text-transform:uppercase;margin-bottom:14px;}
        .doc-cell-dest{flex:1;font-size:12.5px;line-height:1.5;}
        .doc-cell-dest .dest-name{font-weight:700;margin-bottom:2px;}
        .doc-title{text-align:center;font-weight:700;font-size:13px;padding:12px 14px;border-bottom:1.5px solid #16233B;text-transform:uppercase;}
        .doc-subtitle{text-align:center;font-weight:600;font-size:12px;padding:8px 14px;border-bottom:1.5px solid #16233B;background:#F7F5EF;}
        .amounts{padding:16px 14px 22px 14px;}
        .amount-row{display:flex;justify-content:space-between;padding:7px 0;}
        .amount-row.total{border-top:1.5px solid #16233B;font-weight:700;margin-top:4px;}
        .amount-row.grand-total{font-weight:700;font-size:13.5px;margin-top:6px;}
        .amount-label{padding-right:16px;}
        .doc-footer{border-top:1.5px solid #16233B;}
        .doc-footer-row{display:flex;align-items:center;gap:20px;padding:10px 14px;border-bottom:1px solid #C9C2AE;font-size:12px;}
        .doc-footer-row .reg-label{font-weight:700;width:90px;flex-shrink:0;}
        .doc-footer-row .net-a-payer{font-weight:700;margin-left:auto;}
        .lettres{padding:8px 14px;font-size:11px;font-style:italic;color:#3A4558;border-bottom:1px solid #C9C2AE;}
        .iban-line{padding:8px 14px;font-size:11px;color:#2B6CB0;border-bottom:1px solid #C9C2AE;}
        .clause{padding:8px 14px;font-size:9.5px;font-weight:600;text-align:center;color:#16233B;border-bottom:1px solid #C9C2AE;}
        .company-footer{padding:8px 14px 12px 14px;font-size:9.5px;color:#8A93A3;text-align:center;}
      </style></head><body>
      <div class="close-bar">
      ...
      <div class="top-header">
        ...
        <div class="company-box">
          <div class="company-name">SYNERGIE BTP</div>
          <div>${SYNERGIE_BTP_COORDS.adresse}</div>
          <div>SIRET : ${SYNERGIE_BTP_COORDS.siret} - TVA Intracommunautaire : ${SYNERGIE_BTP_COORDS.tvaIntra}</div>
          <div>Tél. : ${SYNERGIE_BTP_COORDS.tel} - email : ${SYNERGIE_BTP_COORDS.email}</div>
        </div>
      </div>
      <div class="doc-box">
        <div class="doc-row">
          <div class="doc-cell doc-cell-label">
            <div class="kind">Avance de démarrage</div>
        ...
        <div class="amounts">
          ...
          <div class="amount-row"><span class="amount-label">Montant H.T. de l'avance de démarrage</span><span>${fmtEUR(ht)}</span></div>
          <div class="amount-row total"><span class="amount-label">Total H.T.</span><strong>${fmtEUR(ht)}</strong></div>
          <div class="amount-row"><span class="amount-label">TVA ${TVA_REGIMES[tvaRegime]?.label || ""}</span><span>${fmtEUR(tva)}</span></div>
          <div class="amount-row grand-total"><span class="amount-label">TOTAL AVANCE DE DEMARRAGE</span><strong>${fmtEUR(ttc)}</strong></div>
        </div>
        <div class="doc-footer">
          <div class="doc-footer-row">
            <span class="reg-label">Règlement</span><span>Comptant</span><span class="net-a-payer">Net à payer (Euros) &nbsp; ${fmtEUR(ttc)}</span>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(22,35,59,0.55)" }}>
      <Card className="w-full max-w-lg p-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-semibold text-base" style={{ color: COLORS.ink }}>Appel d'avance de démarrage</h3>
          <button onClick={onClose}><X size={18} color={COLORS.inkSoft} /></button>
        </div>
        <p className="text-xs mb-4" style={{ color: COLORS.inkSoft }}>
          {marcheDisplayName(marche)} — ce document n'est pas une facture Batigest : c'est l'appel de trésorerie que le client règle directement, avant déduction sur la ligne "acompte" du solde du marché.
        </p>
        <div className="flex flex-col gap-3">
          <Field label="Client (nom + adresse, une ligne par info)">
            <textarea
              value={destinataire}
...
// sur tous ses chantiers) ----------
// Ces pièces sont propres à l'ENTREPRISE sous-traitante, pas à un chantier
// précis (le même sous-traitant peut être engagé sur plusieurs chantiers en
// même temps) — Morgane les dépose une seule fois ici plutôt que de les
// redemander à chaque chantier. Chaque dépôt s'AJOUTE à un historique
// (jamais d'écrasement) : l'ancienne pièce reste consultable en archive une
// fois remplacée par une nouvelle — voir uploadSousTraitantPiece plus bas.
// "dateLabel"/"validityMonths" pilotent le calcul de péremption :
//  - validityMonths défini → la date saisie est la date d'ÉMISSION du
//    document, et la péremption est calculée automatiquement (émission +
//    N mois), comme l'exige la réglementation pour le KBIS/URSSAF/fiscale.
//  - validityMonths = null → la date saisie EST directement la date
//    d'expiration telle qu'elle figure sur le document (assurance, pièce
//    d'identité, titre de séjour : leur durée de validité n'est pas fixe).
const SOUS_TRAITANT_PIECE_TYPES = [
  { key: "kbis", label: "KBIS", dateLabel: "Date d'émission", validityMonths: 3 },
  { key: "urssaf", label: "URSSAF", dateLabel: "Date d'émission", validityMonths: 6 },
  { key: "fiscale", label: "Attestation fiscale", dateLabel: "Date d'émission", validityMonths: 6 },
...
// Clés de stockage pour les pièces du dossier sous-traitant (réutilise
// /api/documents comme partout ailleurs — le champ "chantierId" de cette
// route n'est qu'un préfixe de rangement, il accepte n'importe quel id).
function sousTraitantPieceDocKey(sousTraitantId, type) {
  return "sst-piece-" + sousTraitantId + "-" + type;
}
function salarieEtrangerPieceDocKey(salarieId, type) {
  return "sst-salarie-" + salarieId + "-" + type;
}
// URL officielle de vérification des attestations de vigilance (page
// publique urssaf.fr : on y colle le "code de sécurité" à 15 caractères
// imprimé sur l'attestation, elle confirme si le document est bien valide
// et à jour). Il n'existe pas d'API accessible en un clic pour Morgane (la
// vraie API officielle — API Entreprise — est réservée aux organismes
// habilités et demande une démarche d'accès à part) : on ouvre donc
// simplement cette page et on copie le code dans le presse-papiers pour
// qu'elle n'ait plus qu'à le coller (voir handleVerifyUrssaf).
const URSSAF_VERIF_URL = "https://www.urssaf.fr/accueil/outils-documentation/outils/verification-attestation.html";
async function handleVerifyUrssaf(code) {
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      await navigator.clipboard.writeText(code);
    }
  } catch {
    // Le copier-coller automatique peut être refusé par le navigateur : le
    // code reste de toute façon affiché en clair à côté du bouton, elle
    // peut le sélectionner/copier à la main.
  }
  window.open(URSSAF_VERIF_URL, "_blank");
}

// Idem pour le KBIS : data.inpi.fr (registre national des entreprises) ne
// délivre pas le vrai document KBIS — celui-ci reste réservé aux greffes
// des tribunaux de commerce (Infogreffe/monidenum.fr, payant à chaque
// téléchargement) — mais sa fiche entreprise gratuite (nom, forme
// juridique, dirigeants, statut actif/radié...) suffit déjà à Morgane pour
// vérifier en un clic qu'un sous-traitant est toujours en activité quand
// son KBIS archivé est périmé. Fiche accessible directement par SIREN
// (les 9 premiers chiffres du SIRET saisi dans le dossier) :
// data.inpi.fr/entreprises/{siren} ; à défaut de SIRET exploitable, on
// retombe sur la recherche avancée du site (elle peut alors chercher par
// nom directement dessus).
function sirenFromSiret(siret) {
  const digits = String(siret || "").replace(/\D/g, "");
  return digits.length >= 9 ? digits.slice(0, 9) : "";
}
function inpiCompanyUrl(siret) {
  const siren = sirenFromSiret(siret);
  return siren ? `https://data.inpi.fr/entreprises/${siren}` : "https://data.inpi.fr/recherche_avancee/entreprises";
}
// Dernière pièce déposée pour un type donné (le "dossier courant") — les
// autres entrées du tableau restent des archives consultables mais ne
// comptent plus pour la péremption. Triée sur la date SAISIE par Morgane
// (celle du document), pas sur la date de dépôt dans l'appli.
function currentPieceEntry(entries) {
  if (!entries || !entries.length) return null;
  return [...entries].sort((a, b) => (b.date || "").localeCompare(a.date || ""))[0];
}
// Date d'expiration effective d'une pièce, selon sa règle de validité (voir
// SOUS_TRAITANT_PIECE_TYPES ci-dessus).
function pieceExpiryIso(entry, typeDef) {
  if (!entry || !entry.date || !typeDef) return null;
  if (typeDef.validityMonths == null) return entry.date;
  const d = new Date(entry.date + "T00:00:00");
  if (isNaN(d.getTime())) return null;
  d.setMonth(d.getMonth() + typeDef.validityMonths);
  return d.toISOString().slice(0, 10);
}
// Chantiers sur lesquels ce sous-traitant est actuellement en mission —
// contrat ni annulé, ni déjà terminé (date de fin dépassée). Sert à ne
// déclencher une alerte "pièce périmée" que quand ça a un impact réel.
function sousTraitantActiveChantiers(sousTraitantId, chantiers) {
  const out = [];
  for (const c of chantiers || []) {
    if (c.isFacturesLibres) continue;
    for (const e of c.sousTraitance || []) {
      if (e.sousTraitantId !== sousTraitantId) continue;
      if (e.statutContrat === "annule") continue;
      if (e.dateFin && daysUntil(e.dateFin) < 0) continue;
      out.push({ chantierId: c.id, chantierTitre: c.titre, entryId: e.id });
    }
  }
  return out;
}
// Pièces manquantes/périmées/à renouveler bientôt (<=30j) du dossier
// administratif d'un sous-traitant (voir SOUS_TRAITANT_PIECE_TYPES).
function sousTraitantPieceAlerts(sousTraitant) {
  ...
  for (const typeDef of SOUS_TRAITANT_PIECE_TYPES) {
    ...
    if (!current) {
      out.push({ key: typeDef.key, label: typeDef.label, status: "manquante", expiryIso: null, days: null });
      continue;
    }
    const expiryIso = pieceExpiryIso(current, typeDef);
...
// "caution banque" (m.rgMode === "banque") — support à la fois de l'onglet
// "Caution bancaire" et de la tuile équivalente sur le tableau de bord.
// Un marché est considéré "soldé" (donc à faire lever) dès que son reste à
// facturer HT est nul, exactement comme la ligne "reste à facturer" déjà
// affichée sur la fiche chantier (voir renderMarcheBlock) — pour rester
// cohérent avec ce que Morgane voit déjà là-bas.
...
// Comble les champs manquants sur des sous-traitants déjà enregistrés avant
...
function Dashboard({ chantiers, rgDues, computed, setTab, setSelectedChantier, sousTraitants, onOpenSousTraitantDossier }) {
  ...
  // atteinte ou dans moins de 30j (même fenêtre que "RG à réclamer" ci-dessus),
  // ce qui inclut aussi les cautions soldées où la date du PV de réception
  // n'a même pas encore été saisie (alerteJours === null, donc pas exclue
  // ici : Morgane doit être relancée pour la saisir).
  const cautionsAAnnuler = cautionsBancaires(chantiers).filter((c) => c.soldee);
  const cautionsARelancer = cautionsAAnnuler.filter((c) => c.alerteJours === null || c.alerteJours <= 30);
  ...
  // ou expire sous 30j — la seule situation où Morgane a vraiment besoin
  // d'être alertée (voir sousTraitantActiveChantiers/sousTraitantPieceAlerts).
  const sousTraitantsAlertes = useMemo(() => {
    const out = [];
    for (const s of sousTraitants || []) {
      const actifs = sousTraitantActiveChantiers(s.id, chantiers);
      if (!actifs.length) continue;
      // sousTraitantPieceAlerts ne renvoie déjà que les pièces manquantes,
      // périmées ou expirant sous 30j — jamais celles à jour.
      const pieces = sousTraitantPieceAlerts(s);
      if (!pieces.length) continue;
      out.push({ sousTraitant: s, actifs, pieces });
    }
    return out.sort((a, b) => {
      const aPerimee = a.pieces.some((p) => p.status === "perimee");
      const bPerimee = b.pieces.some((p) => p.status === "perimee");
  ...
  return (
    <div className="p-4 max-w-6xl">
      ...
      <ResponsiveGrid min={160} className="mb-6">
        ...
        <Card className="p-4" style={{ cursor: "pointer" }} onClick={() => setTab("caution")}>
      ...
      {sousTraitantsAlertes.length > 0 && (
        <Card className="p-4 mb-6" style={{ borderColor: COLORS.redSoft, background: COLORS.redSoft }}>
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle size={16} color={COLORS.red} />
            <span className="text-sm font-semibold" style={{ color: COLORS.red }}>Sous-traitants — pièces à renouveler (en mission actuellement)</span>
          </div>
          <div className="flex flex-col gap-1.5">
            {sousTraitantsAlertes.map(({ sousTraitant, actifs, pieces }) => (
              <button
                key={sousTraitant.id}
                onClick={() => onOpenSousTraitantDossier && onOpenSousTraitantDossier(sousTraitant.id)}
                className="text-xs flex flex-col text-left hover:underline"
                style={{ color: COLORS.ink }}
              >
                <span className="font-medium">
                  {sousTraitant.nom} — {actifs.map((a) => a.chantierTitre).join(", ")}
                </span>
                <span style={{ color: COLORS.red }}>
                  {pieces.map((p) => `${p.label} (${p.status === "perimee" ? `périmée depuis ${Math.abs(p.days)} j` : p.status === "manquante" ? "manquante" : `expire dans ${p.days} j`})`).join(" · ")}
                </span>
              </button>
            ))}
          </div>
...
function ChantierDetail({ chantier, updateChantier, unlocked, setTab, onArchiveChantier, sousTraitants, onAddSousTraitant, onOpenSousTraitantDossier }) {
  ...
  // Génération de "l'appel d'avance de démarrage" (le document que le
  // client règle directement, sans facture Batigest en face, avant
  ...
  // chantier (uniquement les infos propres au chantier : DC4, contrat,
  // dates, montant) — voir SS_TRAITANCE_STATUTS / sousTraitanceDocKey plus
  // haut dans le fichier. Les attestations administratives (URSSAF, KBIS,
  ...
  // orphelin laissé sur le stockage.
  async function removeSousTraitanceEntry(id) {
    const docsCur = chantier.documents || {};
    const pieceKeys = ["dc4", "contrat"].map((t) => sousTraitanceDocKey(id, t));
  ...
  // Bulle de dépôt de l'acte de caution bancaire — affichée à côté du mode
  // de RG dès qu'un marché est en "caution banque" (voir renderMarcheBlock).
  // Contrairement à renderMarcheAvanceDocBubble ci-dessus, elle reste visible
  // même sans document déposé (cliquable/glisser-déposer pour l'ajouter) :
  // c'est un dépôt manuel de Morgane, pas un PDF généré par l'appli.
  function renderCautionDocBubble(marcheId) {
    const key = "caution-" + marcheId;
    const meta = getDocMeta(docs, key);
    const isUploading = uploadingDocKey === key;
    const isDragOver = dragOverDocKey === key;
    const clickable = !isUploading && (meta.present || unlocked);
    return (
      <div
        ...
        title={`Acte de caution bancaire${meta.present ? " — " + (meta.fileName || "cliquer pour ouvrir") : unlocked ? " — cliquer ou glisser-déposer le PDF ici" : " — aucun acte déposé"}`}
        className="relative inline-flex items-center justify-center"
        style={{
          width: 24, height: 24, borderRadius: 7,
          border: `1.5px ${meta.present ? "solid" : "dashed"} ${meta.present ? COLORS.green : isDragOver ? COLORS.accent : COLORS.line}`,
      ...
      >
        {unlocked && meta.present && !isUploading && (
          <button
            onClick={(e) => { e.stopPropagation(); removeDocument(key); }}
            title="Retirer l'acte de caution bancaire"
            style={{ position: "absolute", top: -6, right: -6, width: 13, height: 13, borderRadius: 999, background: "#fff", border: `1px solid ${COLORS.red}`, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}
          >
            <X size={8} color={COLORS.red} />
          </button>
        ...
        {isUploading ? (
          ...
          <Landmark size={12} color={meta.present ? COLORS.green : COLORS.inkSoft} />
        )}
      </div>
    );
  }
  ...
  return (
    <div className="p-4 max-w-6xl">
                    ...
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      {entry.sousTraitantId && (
                        <>
                          ...
                          <button
                            onClick={() => onOpenSousTraitantDossier && onOpenSousTraitantDossier(entry.sousTraitantId)}
                            className="text-[11px] font-medium hover:underline flex items-center gap-1"
                            style={{ color: COLORS.accent }}
                            title="Voir le dossier administratif du sous-traitant (attestations, KBIS, URSSAF...)"
                          >
      ...
      {(() => {
        ...
        // Un peu de couleur par type de marché (comme le violet déjà utilisé
        // pour le bloc PRORATA) pour repérer d'un coup d'œil, sur un chantier
        // à plusieurs blocs, ce qui est le marché principal et ce qui est un
        // TS — nom du bloc, pastille "HT marché" et léger liseré du tableau.
        const scheme = MARCHE_COLOR_SCHEME[m.type] || MARCHE_COLOR_SCHEME.principal;
        return (
          <div key={m.id} className="mb-5">
            <div className="flex items-center justify-between mb-1.5 px-0.5 flex-wrap gap-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!isProrata && m.rgMode === "banque" && (
                  <span className="inline-flex items-center gap-1.5">
                    <span className="text-xs" style={{ color: COLORS.inkSoft }}>Acte de caution</span>
                    {renderCautionDocBubble(m.id)}
                  </span>
                )}
              </div>
              {unlocked && (
            ...
            <Card className="overflow-x-auto" style={{ border: `1px solid ${scheme.border}` }}>
              <div style={{ overflowX: "auto" }}>
                <table className="text-xs" style={{ width: "100%", minWidth: 1040 }}>
                  <thead>
                    <tr style={{ color: COLORS.inkSoft, background: scheme.headerBg }}>
...
function SousTraitantsView({ chantiers, sousTraitants, unlocked, setTab, setSelectedChantier, onAddSousTraitant, onUpdateSousTraitant, onRemoveSousTraitant, dossierSousTraitantId, onSetDossierSousTraitantId }) {
  ...
  // modale localement (clic sur une ligne) ET depuis un lien dynamique posé
  // ailleurs (fiche chantier, alerte du tableau de bord) sans jongler avec
  // un effet de synchronisation — cet onglet n'étant monté que quand
  // tab === "soustraitants", la prop est déjà la bonne dès le premier rendu.
  const setDossierModalId = onSetDossierSousTraitantId;
  const dossierSousTraitant = dossierSousTraitantId ? sousTraitants.find((s) => s.id === dossierSousTraitantId) : null;
  ...
  const allContrats = useMemo(() => {
    ...
    for (const c of chantiers) {
      ...
      for (const e of (c.sousTraitance || [])) {
        const sst = sousTraitants.find((s) => s.id === e.sousTraitantId);
        // Les attestations administratives (URSSAF, KBIS, assurance...) sont
        // désormais gérées au niveau du sous-traitant lui-même (dossier),
        // plus par contrat — voir la colonne "Dossier" ci-dessous.
        const pieceAlertes = sst ? sousTraitantPieceAlerts(sst) : [];
        out.push({
          ...e,
          chantierId: c.id,
          chantierTitre: c.titre,
          sousTraitantId: e.sousTraitantId,
  ...
  return (
    <div className="p-4 max-w-6xl">
                    ...
                    return (
                      <tr key={s.id} style={{ borderTop: `1px solid ${COLORS.line}` }}>
                        {editable ? (
                          ...
                          <>
                            <td className="px-3 py-2 font-medium" style={{ color: COLORS.ink }}>
                              <button onClick={() => setDossierModalId(s.id)} className="hover:underline text-left" style={{ color: COLORS.ink }}>{s.nom || "—"}</button>
                            </td>
            ...
            <table className="text-xs" style={{ width: "100%", minWidth: 900 }}>
              <thead>
                <tr style={{ color: COLORS.inkSoft, background: "#F7F5EF" }}>
                  ...
                  <th className="text-center font-medium px-2 py-2">Dossier sous-traitant</th>
                </tr>
              </thead>
              <tbody>
                ...
                {filteredContrats.map((e) => (
                  <tr key={e.id} style={{ borderTop: `1px solid ${COLORS.line}` }}>
                    ...
                    <td className="px-2 py-2 text-center">
                      {e.sousTraitantId ? (
                        <button onClick={() => setDossierModalId(e.sousTraitantId)}>
                          <Pill color={e.pieceAlertes.length === 0 ? "green" : e.pieceAlertes.some((p) => p.status === "perimee") ? "red" : "amber"}>
                            {e.pieceAlertes.length === 0 ? "À jour" : `${e.pieceAlertes.length} à vérifier`}
                          </Pill>
                        </button>
                      ) : "—"}
                    </td>
      ...
      {dossierSousTraitant && (
        <SousTraitantDossierModal
          sousTraitant={dossierSousTraitant}
          chantiers={chantiers}
          unlocked={unlocked}
          onClose={() => setDossierModalId(null)}
          onUpdateSousTraitant={onUpdateSousTraitant}
        />
      )}
    </div>
  );
...
// ---------- Dossier administratif d'un sous-traitant ----------
// Un même sous-traitant pouvant être employé sur plusieurs chantiers en même
// temps, ses pièces administratives (URSSAF, KBIS, assurance, pièce
// d'identité, titre de séjour...) sont gérées ICI, une seule fois par
// entreprise — pas par contrat. Chaque type de pièce garde un historique :
...
function DossierPieceRow({ typeDef, entries, unlocked, uploadingKey, storageKey, siret, onTriggerUpload, onDropFile, onOpenEntry, onRemoveEntry }) {
  ...
  const isNoAlert = !!typeDef.noAlert;
  const expiryIso = !isNoAlert ? pieceExpiryIso(current, typeDef) : null;
  const days = expiryIso ? daysUntil(expiryIso) : null;
  const isUploading = uploadingKey === storageKey;
  // Vérification en un clic sur urssaf.fr : seule l'attestation de
  // vigilance URSSAF porte un "code de sécurité" exploitable (voir
  // handleVerifyUrssaf/URSSAF_VERIF_URL plus haut dans le fichier).
  const isUrssaf = typeDef.key === "urssaf";
  // Idem pour le KBIS : bouton vers la fiche entreprise data.inpi.fr (voir
  // inpiCompanyUrl plus haut) — pratique pour vérifier rapidement qu'un
  // sous-traitant est toujours actif quand son KBIS archivé est périmé.
  const isKbis = typeDef.key === "kbis";
  ...
  if (current) {
    if (isNoAlert) {
      ...
      statusColor = "red";
      statusLabel = `Périmée depuis ${Math.abs(days)} j`;
    } else if (days !== null && days <= 30) {
      statusColor = "amber";
      statusLabel = `Expire dans ${days} j`;
    } else {
      statusColor = "green";
      statusLabel = expiryIso ? `Valide jusqu'au ${fmtDate(expiryIso)}` : "À jour";
    }
  }
  ...
  return (
    <div
      ...
      onDrop={(e) => {
        ...
        if (isUrssaf && !pendingCode) { alert("Indique aussi le code de sécurité (15 caractères, imprimé sur l'attestation) avant de la glisser ici."); return; }
        onDropFile(pendingDate, f, pendingCode);
        setPendingDate("");
        setPendingCode("");
      }}
    >
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <div className="text-[11px]" style={{ color: COLORS.inkSoft }}>
            {current ? `${typeDef.dateLabel} : ${fmtDate(current.date)}${current.fileName ? " — " + current.fileName : ""}` : "Aucun document déposé"}
          </div>
          {isUrssaf && current && current.codeSecurite && (
            <div className="text-[11px] mt-0.5" style={{ color: COLORS.inkSoft }}>Code de sécurité : <span className="font-mono select-all">{current.codeSecurite}</span></div>
          )}
        </div>
      ...
      <div className="flex items-center gap-3 mt-2 flex-wrap">
        {current && (
          <button onClick={() => onOpenEntry(current)} className="text-xs font-medium hover:underline" style={{ color: COLORS.accent }}>Voir le document</button>
        )}
        {isUrssaf && current && current.codeSecurite && (
          <button
            onClick={() => handleVerifyUrssaf(current.codeSecurite)}
            title="Copie le code de sécurité et ouvre la page officielle de vérification sur urssaf.fr"
            className="text-xs font-medium hover:underline flex items-center gap-1"
            style={{ color: COLORS.accent }}
          >
            <ExternalLink size={11} /> Vérifier sur urssaf.fr
          </button>
        )}
        {isKbis && (
          <button
            onClick={() => window.open(inpiCompanyUrl(siret), "_blank")}
            title="Ouvre la fiche entreprise sur data.inpi.fr : statut actif/radié, dirigeants, et un bouton « Télécharger » pour récupérer gratuitement l'extrait RNE en PDF. Ce n'est pas le KBIS du greffe (payant sur Infogreffe/monidenum.fr), mais suffit en général pour vérifier qu'un sous-traitant est toujours en activité."
            className="text-xs font-medium hover:underline flex items-center gap-1"
            style={{ color: COLORS.accent }}
          >
            <ExternalLink size={11} /> Voir sur data.inpi.fr
          </button>
        )}
        {archive.length > 0 && (
          <button onClick={() => setShowHistory((v) => !v)} className="text-xs hover:underline" style={{ color: COLORS.inkSoft }}>
            {showHistory ? "Masquer" : "Voir"} l'historique ({archive.length})
          </button>
        )}
      </div>
      {showHistory && archive.length > 0 && (
        <div className="mt-2 flex flex-col gap-1 pl-2" style={{ borderLeft: `2px solid ${COLORS.line}` }}>
          {archive.map((e) => (
            <div key={e.id} className="flex items-center justify-between text-[11px]" style={{ color: COLORS.inkSoft }}>
              <span>{fmtDate(e.date)} — {e.fileName}</span>
              <div className="flex items-center gap-2">
                <button onClick={() => onOpenEntry(e)} className="hover:underline" style={{ color: COLORS.accent }}>Voir</button>
                {unlocked && <button onClick={() => onRemoveEntry(e)} title="Supprimer définitivement cette archive"><Trash2 size={11} color={COLORS.red} /></button>}
              </div>
            </div>
          ))}
        </div>
      ...
      {unlocked && (
        <div className="flex items-end gap-2 mt-2.5 pt-2.5 flex-wrap" style={{ borderTop: `1px dashed ${COLORS.line}` }}>
          <Field label={typeDef.dateLabel}>
            <TextInput type="date" value={pendingDate} disabled={isUploading} onChange={(e) => setPendingDate(e.target.value)} style={{ width: 145 }} />
          </Field>
          {isUrssaf && (
            <Field label="Code de sécurité (15 car.)">
              <TextInput value={pendingCode} disabled={isUploading} onChange={(e) => setPendingCode(e.target.value.trim())} style={{ width: 165 }} placeholder="ex. AB12CD34EF56GH7" />
            </Field>
          )}
          <Btn
            size="sm"
            ...
            onClick={() => {
              if (isUrssaf && !pendingCode) { alert("Indique aussi le code de sécurité (15 caractères, imprimé sur l'attestation) avant de l'ajouter."); return; }
              onTriggerUpload(pendingDate, pendingCode);
              setPendingDate("");
              setPendingCode("");
            }}
          >
...
function SousTraitantDossierModal({ sousTraitant, chantiers, unlocked, onClose, onUpdateSousTraitant }) {
  const fileInputRef = useRef(null);
  const [pendingUploadTarget, setPendingUploadTarget] = useState(null);
  const [uploadingKey, setUploadingKey] = useState(null);
  const [addingSalarie, setAddingSalarie] = useState(false);
  const [newSalarieNom, setNewSalarieNom] = useState("");
  const [openSalarieId, setOpenSalarieId] = useState(null);

  const documents = sousTraitant.documents || {};
  const salaries = sousTraitant.salariesEtrangers || [];
  const actifs = sousTraitantActiveChantiers(sousTraitant.id, chantiers);

  function triggerUpload(typeKey, date, code) {
    if (!date) { alert("Indique d'abord la date du document avant de l'ajouter."); return; }
    setPendingUploadTarget({ kind: "piece", key: typeKey, date, code });
    fileInputRef.current?.click();
  }
  function triggerSalarieUpload(salarieId, typeKey, date) {
    if (!date) { alert("Indique d'abord la date du document avant de l'ajouter."); return; }
    setPendingUploadTarget({ kind: "salarie", salarieId, key: typeKey, date });
    fileInputRef.current?.click();
  }
  ...
  async function performUpload(target, file) {
    ...
    const storageKey = target.kind === "piece"
      ...
      : salarieEtrangerPieceDocKey(target.salarieId, target.key);
    setUploadingKey(storageKey);
    try {
      const fd = new FormData();
      fd.append("file", file);
      fd.append("chantierId", sousTraitant.id);
      fd.append("docKey", storageKey);
      const res = await fetch("/api/documents", { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Échec de l'envoi");
      // Le code de sécurité (attestation URSSAF uniquement — voir isUrssaf
      // dans DossierPieceRow) n'est conservé que s'il a été saisi ; ne pas
      // écrire un champ codeSecurite vide sur les autres types de pièces.
      const newEntry = {
        id: uid("doc"),
        date: target.date,
        ...
        ...(target.code ? { codeSecurite: target.code } : {}),
      };
      if (target.kind === "piece") {
        const docs = sousTraitant.documents || {};
        const entries = docs[target.key] || [];
        onUpdateSousTraitant(sousTraitant.id, { documents: { ...docs, [target.key]: [...entries, newEntry] } });
      } else {
        const nextSalaries = (sousTraitant.salariesEtrangers || []).map((s) => {
          if (s.id !== target.salarieId) return s;
          const salDocs = s.documents || {};
          const entries = salDocs[target.key] || [];
          return { ...s, documents: { ...salDocs, [target.key]: [...entries, newEntry] } };
        });
        onUpdateSousTraitant(sousTraitant.id, { salariesEtrangers: nextSalaries });
      }
    } catch (err) {
      alert("Erreur lors de l'envoi du document : " + (err.message || err));
    } finally {
  ...
  async function removePieceEntry(typeKey, entry) {
    if (entry.filePath) {
      try { await fetch(`/api/documents?path=${encodeURIComponent(entry.filePath)}`, { method: "DELETE" }); } catch {}
    }
    const docs = sousTraitant.documents || {};
    const entries = (docs[typeKey] || []).filter((e) => e.id !== entry.id);
    onUpdateSousTraitant(sousTraitant.id, { documents: { ...docs, [typeKey]: entries } });
  }
  async function removeSalarieEntry(salarieId, typeKey, entry) {
    if (entry.filePath) {
      try { await fetch(`/api/documents?path=${encodeURIComponent(entry.filePath)}`, { method: "DELETE" }); } catch {}
    }
    const nextSalaries = (sousTraitant.salariesEtrangers || []).map((s) => {
      if (s.id !== salarieId) return s;
      const salDocs = s.documents || {};
      const entries = (salDocs[typeKey] || []).filter((e) => e.id !== entry.id);
      return { ...s, documents: { ...salDocs, [typeKey]: entries } };
    });
    onUpdateSousTraitant(sousTraitant.id, { salariesEtrangers: nextSalaries });
  }

  function addSalarie() {
    const nom = newSalarieNom.trim();
    if (!nom) return;
    const sal = { ...emptySalarieEtranger(), nom };
    onUpdateSousTraitant(sousTraitant.id, { salariesEtrangers: [...(sousTraitant.salariesEtrangers || []), sal] });
    setNewSalarieNom("");
    setAddingSalarie(false);
    setOpenSalarieId(sal.id);
  }
  async function removeSalarie(salarieId) {
    const sal = (sousTraitant.salariesEtrangers || []).find((s) => s.id === salarieId);
    if (sal) {
      const allEntries = Object.values(sal.documents || {}).flat();
      await Promise.all(allEntries.map(async (e) => {
        if (e.filePath) { try { await fetch(`/api/documents?path=${encodeURIComponent(e.filePath)}`, { method: "DELETE" }); } catch {} }
      }));
    }
    onUpdateSousTraitant(sousTraitant.id, { salariesEtrangers: (sousTraitant.salariesEtrangers || []).filter((s) => s.id !== salarieId) });
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(22,35,59,0.55)" }} onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg" style={{ background: COLORS.paper }} onClick={(e) => e.stopPropagation()}>
        <input ref={fileInputRef} type="file" accept="application/pdf,image/*" className="hidden" onChange={handleFileChosen} />
        <div className="p-5">
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-lg font-semibold" style={{ color: COLORS.ink }}>{sousTraitant.nom || "Sous-traitant"}</h3>
            <button onClick={onClose}><X size={18} color={COLORS.inkSoft} /></button>
          </div>
          <p className="text-xs mb-4" style={{ color: COLORS.inkSoft }}>Dossier administratif — pièces à jour requises pour toute intervention en cours.</p>

          {actifs.length > 0 && (
            <div className="text-xs mb-4 px-3 py-2 rounded-md" style={{ background: COLORS.accentSoft, color: COLORS.ink }}>
              Actuellement en mission sur : {actifs.map((a) => a.chantierTitre).join(", ")}
            </div>
          )}
            ...
            {SOUS_TRAITANT_PIECE_TYPES.filter((t) =>
              ...
              return (
                <DossierPieceRow
                  key={typeDef.key}
                  typeDef={typeDef}
                  entries={documents[typeDef.key] || []}
                  unlocked={unlocked}
                  uploadingKey={uploadingKey}
                  storageKey={storageKey}
                  siret={sousTraitant.siret}
                  onTriggerUpload={(date, code) => triggerUpload(typeDef.key, date, code)}
                  onDropFile={(date, file, code) => performUpload({ kind: "piece", key: typeDef.key, date, code }, file)}
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="text-sm font-semibold" style={{ color: COLORS.ink }}>Salariés étrangers</div>
          ...
          {addingSalarie && (
            <div className="flex items-center gap-2 mb-3 p-2 rounded-md" style={{ background: COLORS.accentSoft }}>
              <TextInput autoFocus placeholder="Nom du salarié" value={newSalarieNom} onChange={(e) => setNewSalarieNom(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addSalarie(); }} style={{ flex: 1 }} />
              <Btn size="sm" variant="accent" onClick={addSalarie}>Ajouter</Btn>
              <button onClick={() => { setAddingSalarie(false); setNewSalarieNom(""); }}><X size={14} color={COLORS.inkSoft} /></button>
            </div>
          )}
          {salaries.length === 0 ? (
            <p className="text-xs" style={{ color: COLORS.inkSoft }}>Aucun salarié étranger déclaré pour ce sous-traitant.</p>
          ) : (
            <div className="flex flex-col gap-2">
              {salaries.map((sal) => {
                const isOpen = openSalarieId === sal.id;
                const missingCount = SALARIE_ETRANGER_PIECE_TYPES.filter((t) => !currentPieceEntry((sal.documents || {})[t.key])).length;
                return (
                  <div key={sal.id} className="rounded-lg" style={{ border: `1px solid ${COLORS.line}` }}>
                    <button onClick={() => setOpenSalarieId(isOpen ? null : sal.id)} className="w-full flex items-center justify-between px-3 py-2">
                      <span className="text-sm font-medium flex items-center gap-2" style={{ color: COLORS.ink }}>
                        <Users size={13} /> {sal.nom || "(sans nom)"}
                      </span>
                      <div className="flex items-center gap-2">
                        {missingCount > 0 ? <Pill color="amber">{missingCount} pièce(s) manquante(s)</Pill> : <Pill color="green">Complet</Pill>}
                        <ChevronDown size={14} color={COLORS.inkSoft} style={{ transform: isOpen ? "rotate(180deg)" : "none" }} />
                      </div>
                    </button>
                    {isOpen && (
                      <div className="px-3 pb-3 flex flex-col gap-2">
                        {unlocked && (
                          <div className="flex justify-end">
                            <button onClick={() => removeSalarie(sal.id)} className="text-xs flex items-center gap-1 hover:underline" style={{ color: COLORS.red }}><Trash2 size={12} /> Retirer ce salarié</button>
                          </div>
                        )}
                        {SALARIE_ETRANGER_PIECE_TYPES.map((typeDef) => {
                          const storageKey = salarieEtrangerPieceDocKey(sal.id, typeDef.key);
                          return (
                            <DossierPieceRow
                              key={typeDef.key}
                              typeDef={typeDef}
                              entries={(sal.documents || {})[typeDef.key] || []}
                              unlocked={unlocked}
                              uploadingKey={uploadingKey}
                              storageKey={storageKey}
...
// que le marché n'est pas soldé, puis bascule automatiquement en "Cautions à
...
export default function App() {
  ...
  // savoir quelle modale afficher — état détenu ici pour survivre au
  // changement d'onglet, sans passer par un effet de synchronisation.
  const [pendingDossierSousTraitantId, setPendingDossierSousTraitantId] = useState(null);
  ...
  const openSousTraitantDossier = useCallback((id) => {
    setPendingDossierSousTraitantId(id);
    setTab("soustraitants");
  }, []);
